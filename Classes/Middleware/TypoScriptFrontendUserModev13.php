 ...
 */
class TypoScriptFrontendUserModev13 implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        /** @var $typoscript FrontendTypoScript */
        /** @var $pageInformation PageInformation */
        if (!($typoscript = $request->getAttribute('frontend.typoscript'))
            || !($pageInformation =  $request->getAttribute('frontend.page.information'))
        ) {
            return $handler->handle($request);
        }

        // Initialize the context and store the current user array and the assigned user
        $frontendUser = $request->getAttribute('frontend.user');
        $userArray = $frontendUser->user;
        $userGroups = $frontendUser->userGroups;

        // Check whether login is allowed in the rootline and fetch a possible login mode
        [$loginAllowed, $loginMode] = $this->getLoginModeDetails($pageInformation);
        ...
        if (!$loginAllowed && $this->context->getAspect('frontend.user')->isUserOrGroupSet()) {
            if ($loginMode === 'all') {
                $frontendUser->user = null;
            }
            $frontendUser->userGroups = [];
            $userAspect = $frontendUser->createUserAspect(false);
        ...
        if (($typoscript->getConfigArray()['sendCacheHeaders'] ?? false)
            && ($typoscript->getConfigArray()['sendCacheHeaders_onlyWhenLoginDeniedInBranch'] ?? false)
            && $loginAllowed
        ) {
     ...
     */
    protected function getLoginModeDetails(PageInformation $pageInformation): array
    {
        $loginAllowed = true;
        $loginMode = '';

        // Traverse root line from root and outwards:
        foreach (array_reverse($pageInformation->getRootLine()) as $page) {
